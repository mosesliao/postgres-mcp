...
def test_valid_select(): pass

@scenario("../features/security.feature", "A valid WITH (CTE) query is accepted")
def test_valid_cte(): pass

@scenario("../features/security.feature", "Leading whitespace does not bypass the keyword check")
def test_leading_whitespace(): pass

@scenario("../features/security.feature", "A semicolon prefix does not bypass the keyword check")
def test_semicolon_prefix(): pass

@scenario("../features/security.feature", "A write operation hidden inside a CTE is rejected by the database session")
def test_cte_write_smuggling(): pass

@scenario("../features/security.feature", "The database connection is opened in read-only mode")
def test_connection_readonly(): pass

@scenario("../features/security.feature", "A direct INSERT on a read-only session is rejected by PostgreSQL")
def test_direct_insert_rejected(): pass
...
def db_readonly_error(ctx):
    ...
    assert "read-only" in str(err).lower() or "cannot execute" in str(err).lower(), (
